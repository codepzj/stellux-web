...
import Md from '@/shared/components/Md/ui/md'
...
export function AdminMarkdownEditor({
  ...
  const getPreviewViewport = React.useCallback(() => {
    return (
      previewRootRef.current?.querySelector<HTMLDivElement>('[data-slot="scroll-area-viewport"]') ??
      null
    )
  }, [])
  ...
  const syncPreviewFromEditor = React.useCallback(
    ...
    (command: MarkdownCommand) => {
      ...
      if (command === 'image') {
        ...
        replaceRange(
          ...
          selectionEnd,
          nextText,
          selectionStart + alt.length + 4,
          selectionStart + alt.length + 12
        )
        return
      }
      if (command === 'table') {
    ...
    >
      ...
      <div
        className={cn(
          ...
          mode === 'split' &&
            'grid-rows-2 md:grid-cols-[minmax(0,1fr)_minmax(0,1fr)] md:grid-rows-1',
